module ApplicationHelper
  ...
  def sortable(column, title = nil)
    ...
    link_to "#{title} <i class='#{css_class == "current asc" ? "bts bt-caret-up" : ""} #{css_class == "current desc" ? "bts bt-caret-down" : ""}'></i>".html_safe, {:sort => column, :direction => direction, :search => params[:search], :limit => params[:limit]}, {:class => css_class}
  end
